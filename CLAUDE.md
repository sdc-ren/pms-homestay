...
> ⚠️ **Exception — Playwright: do NOT use `rtk`.** Never run `rtk playwright …`; run Playwright directly (and prefer the `mcp__playwright__*` browser tools, which don't go through rtk at all). RTK compacts/summarizes output and strips the snapshots and detail needed to drive the browser.
